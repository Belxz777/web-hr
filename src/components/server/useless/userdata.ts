...
async function quickSearchUsers(text: string) {
  const res = await fetch(`${host}users/quicksearch/?search=${text}`, {
    credentials: "include",
    method: "GET",
  });
  if (!res.ok) {
...
export { userTasks, userTaskstoReport, deleteUser, quickSearchUsers };
