  ...
  if (!hasFunctions && !hasDeputies) {
    return (
      <div className="bg-card/95 backdrop-blur-sm rounded-xl p-6 mb-6 shadow-lg border border-border">
        <h2 className="text-xl font-bold text-foreground mb-4">Топ функций и обязанностей</h2>
  ...
  return (
    <div className="bg-card/95 backdrop-blur-sm rounded-xl p-6 mb-6 shadow-lg border border-border">
                ...
                >
                  <div className="flex justify-center mb-3">
                    <CircularProgress percentage={func?.percent} color={func.isTypical ? "#32CD32" : "#B22222"}>
                      <div className="text-lg font-bold text-foreground">{func.percent.toFixed(1)}%</div>
                    </CircularProgress>
                  </div>
                  ...
                  <div className="flex justify-between text-sm text-muted-foreground border-t border-border pt-2">
                    ...
                    <div>
                      {func.entries_count} {func.entries_count === 1 ? "запись" : "записи"}
                    </div>
                  </div>
                </div>
              )
      ...
      {hasDeputies && (
        <>
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-foreground">Топ дополнительных обязанностей</h2>
            <div className="text-sm text-muted-foreground">
                ...
                >
                  <div className="flex justify-center mb-3">
                    <CircularProgress percentage={deputy?.percent} color="#F0E68C">
                      <div className="text-lg font-bold text-foreground">{deputy.percent.toFixed(1)}%</div>
                    </CircularProgress>
                  </div>
                  <div className="text-center mb-3">
                    <div className="font-medium text-foreground line-clamp-2 h-12 flex items-center justify-center">
                  ...
                  <div className="flex justify-between text-sm text-muted-foreground border-t border-border pt-2">
                    <div className="font-medium">{convertedTime}</div>
                    <div>
                      {deputy.entries_count} {deputy.entries_count === 1 ? "запись" : "записи"}
                    </div>
                  </div>
                </div>
              )
      ...
      {!hasFunctions && hasDeputies && (
        <div className="bg-secondary/10 border border-secondary/20 rounded-lg p-4 mb-6">
          ...
          <div className="text-muted-foreground text-sm">Нет данных по основным функциям для отображения</div>
        </div>
      )}

      {hasFunctions && !hasDeputies && (
        <div className="bg-primary/10 border border-primary/20 rounded-lg p-4">
          <div className="text-primary font-medium">Дополнительные обязанности не найдены</div>
          <div className="text-muted-foreground text-sm">Нет данных по дополнительным обязанностям для отображения</div>
        </div>
      )}
    </div>
  )
