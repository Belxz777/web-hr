...
import { analyticsDepartments, getCommonDepartments } from "@/components/server/analysis/departmentanalysis"
...
export default function GeneralDashboard() {
  const [depsGeneralData, setDepsGeneralData] = useState<DepartmentsData>({ 
    departments: [], 
    total_departments: 0 
  })
  ...
  const fetchDepsGeneral = async (date: string) => {
    ...
    try {
      const data = await getCommonDepartments({ date })
      if (!data || !data.departments) {
        throw new Error("Неверный формат данных")
      }
      setDepsGeneralData(data)
    } catch (error: any) {
      console.error("Получить данные не получилось", error)
      setError(error.message || "Ошибка при загрузке данных")
      setDepsGeneralData({ departments: [], total_departments: 0 })
    } finally {
      setLoading(false)
    }
  }
  ...
  const hasData = depsGeneralData.departments.some(
    (dept) => dept.department_stats.total_hours > 0 || dept.department_stats.employee_count > 0
  )

  const displayDate = depsGeneralData.departments[0]?.time_period.date || selectedDate
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm mb-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              <h2 className="text-xl font-bold text-[#000000] mb-4 md:mb-0">
                Статистика по отделам за {formatDisplayDate(displayDate)}
              </h2>
            ...
            {error && (
              <div className="mt-4 text-red-500 text-sm">
                {error}
                <button 
                  onClick={() => fetchDepsGeneral(selectedDate)}
          ...
          {loading ? (
            <div className="flex items-center justify-center h-64">
              <LoadingOverlay isVisible={loading} message="Загрузка данных..." />
            </div>
          ) : (
            <>
              <SummaryStats data={depsGeneralData} />

              <div className="mb-6">
                <h3 className="text-xl font-bold text-[#000000] mb-4">
                  Отделы ({depsGeneralData.total_departments})
                </h3>
                {hasData ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {depsGeneralData.departments.map((department) => (
                      <DepartmentCard 
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          {["ID", "Название", "Всего часов", "Функции", "Доп. обязанности", "Сотрудников"].map((header) => (
                            <th
                              key={header}
                              scope="col"
